...
export default function RiskPageClient({ data, totalSuppliers, tab }: RiskPageClientProps) {
  const [activeTab, setActiveTab] = useState(
    tab && !Array.isArray(tab) && operations.some((o) => o.value === tab) ? tab : 'overview'
  );
        ...
        <div className="h-auto overflow-y-auto">
          {operations.map(({ value, content: Content }) => (
            <TabsContent value={value} key={value}>
              <Content riskEntry={data} totalSuppliers={totalSuppliers} setActiveTab={setActiveTab} />
            </TabsContent>
          ))}
        </div>
      </Tabs>
